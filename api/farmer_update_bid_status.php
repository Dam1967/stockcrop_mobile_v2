...
try {
    if ($action === 'accept') {
        ...
        $stmt = $conn->prepare("INSERT INTO notifications (userId, userType, title, message) VALUES (?, 'customer', 'Bid Accepted!', ?)");
        $stmt->bind_param("is", $customerId, $notifMsg);
        $stmt->execute();
        $stmt->close();
        ...
        // Notify customer
        $customerId = intval($bid['customerId']);
        $stmt = $conn->prepare("INSERT INTO notifications (userId, userType, title, message) VALUES (?, 'customer', 'Bid Rejected', 'The farmer rejected your bid')");
        ...
        $stmt = $conn->prepare("INSERT INTO notifications (userId, userType, title, message) VALUES (?, 'customer', 'Counter Offer', ?)");
        $stmt->bind_param("is", $customerId, $notifMsg);
        $stmt->execute();
        $stmt->close();
    ...
    $conn->rollback();
    echo json_encode(['success' => false, 'message' => 'Failed to process: ' . $e->getMessage()]);
}
